...
# F_PATH = Modles/features
F_PATH = FEATURES_PATH

SOM_PATH = "/Users/mayanka/Desktop/GuRu/MaStErS/ThEsIs/sErVeR_SiDe/Som_clusteredData For IHear"
...
def generateFeatures():
    if os.path.exists(F_PATH):
        print("F_PATH Already available")
        return

    files = sc.textFile("sounds_full.txt")
    print("Total file : " + str(files.count()))
    features = files.map(fetchFeatures)
    print("Features Count in :" + features.count())
    print("Saving features to:"+F_PATH)
...
def test(sc):
    ...
    for f in files:
        ...
        print("vecotor is:" +vec)
        testfeatures = Vectors.dense([float(x) for x in vec.split(' ')])
        print("testFeature  is :"+str(testfeatures))
